...
import midi_manipulation_sus
import numpy as np
lowest_note = midi_manipulation_sus.lowerBound #the index of the lowest note on the piano roll
highest_note = midi_manipulation_sus.upperBound #the index of the highest note on the piano roll
...
def run():
    ...
    with tf.Session() as sess:
        saver=tf.train.Saver()
        saver.restore(sess,"mymodels/suspense.ckpt")
        print("restored")
        ...
        for i in range(sample.shape[0]):
            ...
            midi_manipulation_sus.noteStateMatrixToMidi(S, "l/generated_chord_{}".format(i))
            print('saved')
            sess.close()
